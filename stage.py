        ...
        if len(result.chain) > 0:
            ...
            if (
                ...
                if not tts_provider:
                    ...
                    new_chain = []
                    for comp in result.chain:
                        # 检查消息段是否是Plain类型，并且是否被 <tts> 标签包裹
                        if isinstance(comp, Plain) and comp.text.strip().startswith("<tts>") and comp.text.strip().endswith("</tts>"):
                            # 提取标签内的文本内容
                            text_to_convert = comp.text.strip()[5:-6]

                            if len(text_to_convert) > 0:
                                try:
                                    logger.info(f"TTS 请求 (已标记): {text_to_convert}")
                                    audio_path = await tts_provider.get_audio(text_to_convert)
                                    logger.info(f"TTS 结果: {audio_path}")
                                    
                                    if not audio_path:
                                        logger.error(
                                            f"由于 TTS 音频文件未找到，消息段转语音失败: {text_to_convert}"
                                        )
                                        # 转换失败，将原始文本(去除标签后)发送出去
                                    ...
                                    # 转换异常，也将原始文本(去除标签后)发送出去
                                    new_chain.append(Plain(text_to_convert))
                            else:
                                # 如果标签内为空，则什么都不做，忽略这个消息段
                                pass
                        else:
                            # 如果不是被 <tts> 标签包裹的 Plain 文本，或者根本不是 Plain 文本，则直接保留原样
                            new_chain.append(comp)
                    result.chain = new_chain
